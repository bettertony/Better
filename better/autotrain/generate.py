...
def add_packages(config, repeat=1):
    ...
    package_dir = path.realpath(__file__).replace('better/autotrain/generate.pyc',train_dir)\
        .replace("better\\autotrain\\generate.pyc", train_dir)\
                  .replace('better/autotrain/generate.py',train_dir)\
        .replace("better\\autotrain\\generate.py", train_dir)
