...
from better.tools.configprocess import load_config
from better.tools.trade import save_test_data
from better.tools.shortcut import execute_backtest
from better.resultprocess import plot
...
def build_parser():
    ...
    parser.add_argument("--folder", dest="folder", type=int,
                        help="folder(int) to load the config, neglect this option if loading from ./better/net_config")
    return parser
...
def main():
    ...
    if options.mode == "train":
        import better.autotrain.training
        if not options.algo:
            better.autotrain.training.train_all(int(options.processes), options.device)
        else:
            for folder in options.train_floder:
                raise NotImplementedError()
    elif options.mode == "generate":
        import better.autotrain.generate as generate
        logging.basicConfig(level=logging.INFO)
        generate.add_packages(load_config(), int(options.repeat))
    elif options.mode == "download_data":
        from better.marketdata.datamatrices import DataMatrices
